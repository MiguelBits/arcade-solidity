  ...
  handleNetworkSwitch = async () => {
    try {
      ...
      let txn = await window.ethereum.request({
          ...
          params: [this.state.network["arbitrumRinkeby"]]
      });
      await txn.wait();
    } catch (err) {
      console.log(err)
  ...
  mintToken = async () =>{
    const { ethereum } = window;
      if (ethereum) {
        ...
        tokenContract.saveThatMoney().then(balance =>{
